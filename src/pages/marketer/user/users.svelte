...
const fetchMarketer =async()=>{
    ...
    .then((datas) => {
        distributers = datas.data.distributer;
...
<div class="container pt-4">
    <div class="border">
        <h4 class="heading text-center p-2 border-bottom">Distributers</h4>
        <div class="row justify-content-center">
            {#each distributers as user}
                <a class="col-sm-3 card card-user" href="/marketer/user_list/{user._id}">
                    <div class="card-info">
                        <h4>{user.name}</h4>
                        <h6>Phone: <b>{user.phone}</b></h6>
                        <p>Status: <b>{user.active}</b></p>
                    </div>
                </a>
            {/each}
        </div>
